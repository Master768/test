...
async def create_room(request: CreateRoomRequest):
    print(f"DEBUG: Creating room '{request.room_name}' with host_name='{request.host_name}' and exchange_date='{request.exchange_date}'")
    
    # Check if this host already has a room with the same name
    if request.room_name:
        query = {
            "host_name": request.host_name,
            "name": request.room_name
        }
        print(f"DEBUG: Searching for existing room with query: {query}")
        existing_room = await db.db.rooms.find_one(query)
        
        if existing_room:
            print(f"DEBUG: Found existing room {existing_room['code']} for host {request.host_name}")
            # Return the existing room
            return Room(**existing_room)
        else:
            print(f"DEBUG: No existing room found for host '{request.host_name}' with name '{request.room_name}'")
    ...
    room = Room(
        code=room_code, 
        name=request.room_name,
        host_name=request.host_name,  # Store the host name
        participants=[host],
        exchange_date=None  # Date is set later
    )
    
    print(f"DEBUG: Inserting new room: code={room.code}, name={room.name}, host_name={room.host_name}")
...
async def join_room(request: JoinRequest):
    ...
    for participant in room.participants:
        if participant.name.lower() == request.name.lower():
            existing_participant = participant
            break
    
    if existing_participant:
        print(f"DEBUG: Found existing participant {existing_participant.id} with name '{request.name}'")
        
        # Update their preferences and secret message if new ones are provided
        if request.preferences or request.secret_message:
            await db.db.rooms.update_one(
                {"code": request.room_code, "participants.id": existing_participant.id},
                {"$set": {
                    "participants.$.preferences": request.preferences or existing_participant.preferences,
                    "participants.$.secret_message": request.secret_message or existing_participant.secret_message
                }}
            )
            # Update the local object
            existing_participant.preferences = request.preferences or existing_participant.preferences
            existing_participant.secret_message = request.secret_message or existing_participant.secret_message
        
        return existing_participant
        
    # Create new participant if name doesn't exist
